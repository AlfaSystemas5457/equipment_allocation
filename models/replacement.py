...
class Replacement(models.Model):
    ...
    uid_replacement = fields.Many2one(
        ...
        tracking=True,
        required=True,
        domain="[('state', 'not in', ['rejected', 'returned', 'draft']), ('has_replacement', '=', False)]",
        ondelete="restrict",
    )
    ...
    def create(self, vals):
        if not vals.get("uid"):
            vals["uid"] = (
                self.env["ir.sequence"].next_by_code("equipment.replacement") or "/"
            )
        ...
        if vals.get("uid_replacement"):
            ...
            if allocation:
                ...
                vals.setdefault("equipment_ids", [(6, 0, allocation.equipment_ids.ids)])
            ...
            for record in self:
                ...
                if allocation:
                    ...
                    if not record.equipment_ids:
                        record.equipment_ids = [(6, 0, allocation.equipment_ids.ids)]
    ...
    def adding_data(self):
        ...
        allocation = self.env["equipment.allocations"].search(
            [("uid", "=", self.uid_replacement.uid)], limit=1
        )
    ...
    def handle_rejected(self):
        ...
        # posible bug
        if replacement:
            replacement.write({"has_replacement": False})
